...
```{r, warning=F}
this.label <- c('Older men', 'Older women', 'Younger men', 'Younger women')
age.indicator <- c(0,0,1,1)
gender.indicator <- c(1,0,1,0)
set.seed(1)
CONT <- NULL
for(j in 1:length(this.label)){
  ...
  print(kable(est_sub$compact, caption=paste0('Subgroup results for ', this.label[j],
              ' (N=', nrow(data_sub), ')') ) )
  CONT <- rbind(CONT, cbind(group=this.label[j], est=rownames(est_sub$est),
                            est_sub$est))
...
Comparative results with a **binary outcome** for arm-specific outcomes and intervention effect. 
...
for(j in 1:length(this.label)){
  ...
  print(kable(est_sub$compact, caption=paste0('Subgroup results for ', this.label[j],
              ' (N=', nrow(data_sub), ')') ) )
  BIN <- rbind(BIN, cbind(this.label[j], est_sub$est))
...
```{r, echo=F, include=F, results='hide'}
library('xtable')
x <- rbind(c('Continuous',rep('',ncol(yay.cont)-1)), yay.cont, 
           c('Binary',rep('',ncol(yay.bin)-1)),  yay.bin)
x <- cbind( est=rep(c('','Unadj','Stat', 'Small TMLE', 'Small CTMLE', 'Large TMLE', 'Large CTMLE'),2),x)
